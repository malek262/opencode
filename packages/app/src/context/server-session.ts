  ...
  const applyMessagePage = (
    ...
    const source = page.source
      ? (() => {
          ...
          // Shallow-copy store reads: normalizeSessionMessages caches by message identity, and
          // store proxies keep a stable identity while their content mutates in place. Fresh
          // top-level objects guarantee cache misses (and no proxy-keyed entries) per page.
          const existing = (data.session_message[sessionID] ?? []).map((message) => ({ ...message }))
