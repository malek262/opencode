...
import { createUndiciFetch } from "./util/undici-dispatcher"
...
function prepareOptions(model: ModelV2.Info, pkg: string) {
  ...
  const nodeFetch = typeof customFetch === "function" ? undefined : createUndiciFetch({ chunkTimeout })
  options.fetch = async (input: Parameters<typeof fetch>[0], init?: RequestInit) => {
    ...
    const res = await (customFetch ?? nodeFetch ?? fetch)(input, {
      ...opts,
      timeout: false,
    } as RequestInit)
