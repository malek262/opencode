...
import { FIRST_LAUNCH_ONBOARDING_COMPLETE_KEY, OLD_LAYOUT_ELIGIBLE_KEY, SETTINGS_STORE } from "./store-keys"
...
export function initializeOldLayoutEligibility(userDataPath: string) {
  ...
  const current = storeGet(SETTINGS_STORE, OLD_LAYOUT_ELIGIBLE_KEY)
  if (typeof current === "boolean") return current

  const eligible = hasExistingAppState(entries)
  storeSet(SETTINGS_STORE, OLD_LAYOUT_ELIGIBLE_KEY, eligible)
  return eligible
}

export function isOldLayoutEligible() {
  return storeGet(SETTINGS_STORE, OLD_LAYOUT_ELIGIBLE_KEY) === true
}

export function isFirstLaunchOnboardingPending() {
  const pending = storeGet(SETTINGS_STORE, FIRST_LAUNCH_ONBOARDING_COMPLETE_KEY) !== true
...
export async function finishFirstLaunchOnboarding(createDefaultProject: boolean) {
  ...
  storeSet(SETTINGS_STORE, FIRST_LAUNCH_ONBOARDING_COMPLETE_KEY, true)
